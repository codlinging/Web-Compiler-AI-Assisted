...
pub fn generate_c_code(ast: &ASTNode) -> String {
    match ast {
        ASTNode::FlexFile { rules } => {
            ...
            code.push_str("    char* yytext = \"\"; /* Mock input buffer */\n\n");
            
            for rule in rules {
                if let ASTNode::FlexRule { pattern, action } = rule {
                    code.push_str(&format!("    /* Match Pattern: {} */\n", pattern));
                    code.push_str(&format!("    if (/* regex matches {} */) {{\n", pattern));
        ...
        ASTNode::BisonFile { declarations, rules } => {
            ...
            // 1. Generate token #defines (Bison usually starts at 258)
            ...
            // 2. Generate grammar rule structures
            for rule in rules {
